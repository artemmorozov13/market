...
export class AddTimeCreateAndUpdate1743330880909 implements MigrationInterface {
    name = 'AddTimeCreateAndUpdate1743330880909'

    public async up(queryRunner: QueryRunner): Promise<void> {
        await queryRunner.query(`CREATE TYPE "public"."order_status_enum" AS ENUM('waitForPay', 'payConfirm', 'finished')`);
        await queryRunner.query(`CREATE TABLE "order" ("id" SERIAL NOT NULL, "status" "public"."order_status_enum" NOT NULL DEFAULT 'waitForPay', "created_at" TIMESTAMP NOT NULL DEFAULT now(), "updated_at" TIMESTAMP NOT NULL DEFAULT now(), "address" character varying NOT NULL, "userId" integer, CONSTRAINT "PK_1031171c13130102495201e3e20" PRIMARY KEY ("id"))`);
        await queryRunner.query(`CREATE TABLE "ordered_products_entity" ("id" SERIAL NOT NULL, "telegram_id" integer NOT NULL, "quantity" integer NOT NULL DEFAULT '1', "productId" integer, "userId" integer, "orderId" integer, CONSTRAINT "PK_bf5eb7175f9ef258e7c62c56ac3" PRIMARY KEY ("id"))`);
        await queryRunner.query(`CREATE TABLE "product" ("id" SERIAL NOT NULL, "name" character varying NOT NULL, "description" character varying NOT NULL, "price" numeric(10,2) NOT NULL, "discount" numeric(5,2) NOT NULL, "image" character varying NOT NULL, "createdAt" TIMESTAMP NOT NULL DEFAULT now(), "updatedAt" TIMESTAMP NOT NULL DEFAULT now(), "unitValue" integer NOT NULL, "unitOfMeasurement" character varying NOT NULL, CONSTRAINT "PK_bebc9158e480b949565b4dc7a82" PRIMARY KEY ("id"))`);
        await queryRunner.query(`CREATE TABLE "selected-products" ("id" SERIAL NOT NULL, "productId" integer NOT NULL, "quantity" integer NOT NULL DEFAULT '1', "userTgchatId" integer NOT NULL, "basketId" integer, "userId" integer, CONSTRAINT "PK_32014ad6b5208e4be7508ebbeef" PRIMARY KEY ("id"))`);
        await queryRunner.query(`CREATE TABLE "basket" ("id" SERIAL NOT NULL, "products_count" integer NOT NULL DEFAULT '0', "telegram_id" integer NOT NULL, "userId" integer, CONSTRAINT "REL_26dcb999420495bb5b14a4f8d1" UNIQUE ("userId"), CONSTRAINT "PK_895e6f44b73a72425e434a614cc" PRIMARY KEY ("id"))`);
        await queryRunner.query(`CREATE TABLE "users" ("id" SERIAL NOT NULL, "telegram_id" integer NOT NULL, "telegram_username" character varying NOT NULL DEFAULT '', "name" character varying NOT NULL, "phone_number" character varying NOT NULL DEFAULT '', "is_phone_confirmed" boolean NOT NULL DEFAULT false, "email" character varying NOT NULL, "age" integer NOT NULL DEFAULT '0', "password" character varying NOT NULL DEFAULT '', "created_at" TIMESTAMP NOT NULL DEFAULT now(), "updated_at" TIMESTAMP NOT NULL DEFAULT now(), CONSTRAINT "PK_a3ffb1c0c8416b9fc6f907b7433" PRIMARY KEY ("id"))`);
        ...
        await queryRunner.query(`CREATE TABLE "pickup_point" ("id" SERIAL NOT NULL, "name" character varying NOT NULL, "createdAt" TIMESTAMP NOT NULL DEFAULT now(), "updatedAt" TIMESTAMP NOT NULL DEFAULT now(), CONSTRAINT "PK_9433b0417c214a3cd2fd39d402c" PRIMARY KEY ("id"))`);
        await queryRunner.query(`CREATE TABLE "delivery_time" ("id" SERIAL NOT NULL, "startTime" TIME NOT NULL, "endTime" TIME NOT NULL, "createdAt" TIMESTAMP NOT NULL DEFAULT now(), "updatedAt" TIMESTAMP NOT NULL DEFAULT now(), "deliveryAreaId" integer, CONSTRAINT "PK_ef1b3d23f93ec33cfa77e3bb08d" PRIMARY KEY ("id"))`);
        ...
        await queryRunner.query(`ALTER TABLE "delivery_time" ADD CONSTRAINT "FK_e440fca8c08264e26b41a8aa42d" FOREIGN KEY ("deliveryAreaId") REFERENCES "pickup_point"("id") ON DELETE NO ACTION ON UPDATE NO ACTION`);
    }
