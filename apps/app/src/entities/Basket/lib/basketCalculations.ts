...
export const calculateStoreStats = (groupedBasket: Record<number, GroupedBasketItem>) => {
  return Object.entries(groupedBasket).reduce(
    (acc, [storeId, storeGroup]) => {
...
export const calculateDeliveryCost = (storeTotalPrice: number, store: StoreBaseType) => {
  return storeTotalPrice >= Number(store.deliveryFreeFromLimit) ? 0 : Number(store.deliveryCost)
}
...
export const calculateTotalDiscount = (basket?: BasketBaseType[]) => {
  ...
  return basket.reduce((acc, item) => {
    ...
    const price = item.product.price
    return discount > 0 ? acc + price * item.quantity * (discount / 100) : acc
  }, 0)
}
