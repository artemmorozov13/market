...
const faqs = [
  {
    ...
    answer:
      'Курьер привезёт ваш заказ по указанному адресу, к парадной. Перед доставкой он свяжется с вами для подтверждения. Стоимость доставки — 100 рублей.',
  },
  {
    question: 'По каким дням осуществляется доставка?',
    answer:
      'Дни доставки зависят от района. При оформлении заказа мы автоматически определяем ваш район и предлагаем доступные дни и временные интервалы.',
  },
  {
    question: 'Когда можно сделать заказ?',
    answer:
      'Информация о новых товарах публикуется в Telegram-группах, а также рассылается пользователям этого приложения. Заказ можно изменить или отменить не позднее чем за час до начала дня доставки (до 22:59 предыдущего дня).',
  },
  {
        ...
        <div className={styles.accordions}>
          {faqs.map((faq, index) => (
            <Accordion key={index} className={styles.accordion}>
              ...
              >
                <Typography className={styles.accordionQuestion}>{faq.question}</Typography>
              </AccordionSummary>
              <AccordionDetails>
                <Typography className={styles.accordionAnswer}>{faq.answer}</Typography>
              </AccordionDetails>
            </Accordion>
          ))}
        </div>
