...
import { Typography, Box, Divider, Button, Paper, Avatar, Stack, Chip } from '@mui/material'
        ...
        <Box className={styles.workingHours}>
          {groupedHours.map((group, index) => (
            <Box key={index} className={styles.workingHoursItem}>
              <Typography className={styles.workingHoursDays}>{group.days.join(', ')}</Typography>
              <Typography className={styles.workingHoursTime}>{group.time}</Typography>
            </Box>
          ))}
        </Box>
      </Box>
    ...
    if (isCanceledByAdmin) {
      return (
        <Box className={styles.statusBanner}>
          ...
          {order.cancelReason && (
            <Typography variant="body2">Причина: {order.cancelReason}</Typography>
          )}
          <Typography variant="body2" color="text.secondary">
            Дата отмены: {new Date(order.updatedAt).toLocaleDateString('ru-RU')}
          </Typography>
          ...
          {isActive && (
            <>
              ...
              <Stack direction="row" justifyContent="space-between">
                ...
                <Typography variant="body2">
                  {order.deliveryTime
                    ? `${order.deliveryTime.startTime.slice(0, 5)}-${order.deliveryTime.endTime.slice(0, 5)}`
                    : 'Не указано'}
                </Typography>
              </Stack>
            </>
                  ...
                  <Box sx={{ flex: 1 }}>
                    ...
                    <Box sx={{ display: 'flex', alignItems: 'center', gap: 1, mt: 0.5 }}>
                      {hasDiscount ? (
                        ...
                        <Typography variant="body2">{formatRubbles(price)}</Typography>
                      )}
                    </Box>
                    <Typography variant="body2" color="text.secondary" mt={0.5}>
                      {item.quantity} шт. ×{' '}
                      {hasDiscount ? formatRubbles(discountedPrice) : formatRubbles(price)} ={' '}
            ...
            {order.ordered_products.some((item) => Number(item.product.discount) > 0) && (
              <Stack direction="row" justifyContent="space-between">
                ...
                <Typography variant="body2" color="error.main">
                  -
                  {formatRubbles(
                    order.ordered_products.reduce((sum, item) => {
                      const price = Number(item.product.price)
                      const discount = Number(item.product.discount) || 0
