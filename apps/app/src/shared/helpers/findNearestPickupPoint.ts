...
export const findNearestDeliveryArea = (address: AddressType, points: DeliveryAreaBase[]) => {
  ...
  const availablePoints = pointsWithDistance.filter((point) => point.distance < point.radius / 1000)

  const nearestPoint = [...availablePoints].sort((a, b) => a.distance - b.distance)[0]
  if (nearestPoint) return nearestPoint

  return pointsWithDistance.find((point) => point.name === DEFAULT_STATIC_PICKUP_POINT_NAME) || null
}
