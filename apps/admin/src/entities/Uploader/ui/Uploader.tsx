...
export const Uploader: FC<UploaderProps> = (props) => {
  const { value, onChange, allowedFileTypes = ['image/*'], maxFileSizeMB = 10 } = props
        ...
        <Box className={styles.fileInfoContainer}>
          ...
          <Box className={styles.fileDetails}>
            ...
            <Stack direction="row" spacing={1} mt={1}>
              ...
              {fileInfo?.size && <Chip label={formatFileSize(fileInfo.size)} size="small" />}
              <Chip icon={<CheckCircleIcon />} label="Загружено" color="success" size="small" />
            </Stack>
          </Box>
        </Box>
      )}
