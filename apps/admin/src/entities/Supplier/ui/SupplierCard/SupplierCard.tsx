          ...
          <List>
            {products?.map((product) => (
              <Box key={product.id}>
                <ListItem>
                  <ListItemAvatar>
                    <Avatar src={product.image || undefined} alt={product.name} variant="rounded">
                      <Inventory />
                    </Avatar>
                  </ListItemAvatar>
                  <ListItemText
                    ...
                    secondary={
                      <>
                        ...
                        <Typography component="span" display="block">
                          {formatRubbles(product.offeredPrice)} / {product.unitValue}{' '}
...
export const SupplierCard: FC<SupplierCardProps> = (props) => {
  ...
  const productStats = {
    accepted:
      products?.filter((product) => product.status === ProductStatusEnum.Accepted).length || 0,
    pending:
      products?.filter((product) => product.status === ProductStatusEnum.Moderation).length || 0,
    rejected:
      products?.filter((product) => product.status === ProductStatusEnum.Rejected).length || 0,
    revoked:
      products?.filter((product) => product.status === ProductStatusEnum.Revoked).length || 0,
  }
